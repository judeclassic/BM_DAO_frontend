...
import { approveChatTask, approveRaidTask, getAllModeratorServices, getAllModeratorTaskRaids, getAllRaiderServices, loadChatterToModerate, loadRaidToModerate, rejectChatTask, rejectRaidTask } from "@/app/api/service";
...
const TaskDetails: React.FC<IProps> = ({ id }) => {
    ...
    const handleMarkAsComplete = () => {
        ...
        approveRaidTask({
            raidId: task?.[currentTaskIndex]?._id,
        }).then((res) => {
        ...
        <RightColumn style={{ marginTop: "10px" }}>
            <TaskSub>Raider</TaskSub>
             <Details>
                ...
                <div>
                    <p>Social Media:</p>
                    <BoldP>{task?.[currentTaskIndex]?.taskId?.raidInformation?.action}</BoldP>
                </div>
                <div>
                    <p>Social Media Link:</p>
                    <BoldP>{task?.[currentTaskIndex]?.taskId?.raidInformation?.raidLink}</BoldP>
                </div>
                <div>
